...
void ParallelFor(size_t from, size_t to, Func &&func, size_t grainSize = 1) {
  ...
  static_assert(TBB_MODE != TBB_RAPID,
                "TBB_RAPID is supported by benchmarks/harness_parallel.h");
#else
...
#ifdef EIGEN_MODE
  EigenPartitioner::ParallelFor<EIGEN_STEALING>(0, threadsNum, [&barrier](size_t) {
#else
...
#if 0
  // The working Rapid Start prototype lives in benchmarks/harness_parallel.h.
